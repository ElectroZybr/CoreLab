...
void Camera::zoomAt(float factor, sf::Vector2i mousePixel)
{
    ...
    {
        const sf::Vector2i screenCenter(static_cast<int>(m_screenSize.x * 0.5f), static_cast<int>(m_screenSize.y * 0.5f));
        const sf::Vector2i deltaPixel = mousePixel - screenCenter;
        m_position += sf::Vector2f(static_cast<float>(deltaPixel.x), static_cast<float>(deltaPixel.y)) * (0.1f / m_zoom) * factor;
    }
}
...
sf::Vector2f Camera::screenToWorld(sf::Vector2i screenPos) const
{
    ...
    return {
        viewCenter.x + (static_cast<float>(screenPos.x) - m_screenSize.x * 0.5f) * (viewSize.x / m_screenSize.x),
        viewCenter.y + (static_cast<float>(screenPos.y) - m_screenSize.y * 0.5f) * (viewSize.y / m_screenSize.y)
    };
}
