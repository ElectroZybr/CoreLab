...
void MachineController::seedDemoStruct() {
    ...
    const std::size_t structCount =
        simulation.getRam().getSizeInBytes() / (kStructFloatCount * sim::RAM::kFloatSizeInBytes);
    for (std::size_t structIndex = 0; structIndex < structCount; ++structIndex) {
        ...
        simulation.getRam().writeFloatArray(structAddress, values);
        simulation.getRam().setFloatLabels(structAddress, labels);
    }
}
