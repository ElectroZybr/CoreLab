...
sf::Vector2f computeRamSize(std::size_t slotCount)
{
    ...
    const float slotsHeight =
        ...
        static_cast<float>(rows) * kRowGap;

    return {
        std::max(kMinimumWidth, kLeftPadding + slotsWidth + kRightPadding),
...
)
{
    ...
    for (std::size_t step = 0; step <= kTrackTurnSegments; ++step)
    {
        ...
        bend.append(sf::Vertex(
            {
                center.x + cosine * outerRadius,
                center.y + sine * outerRadius
            },
            color
        ));
        bend.append(sf::Vertex(
            {
                center.x + cosine * innerRadius,
                center.y + sine * innerRadius
            },
            color
        ));
    }
...
void RAM::layout()
{
    ...
    const float lastLaneCenterY =
        m_position.y + kSlotsTopOffset +
        static_cast<float>(rows - 1) * (kSlotSize.y + kRowGap) +
        kSlotSize.y + (kRowGap - kSlotSize.y) * 0.5f + kSlotSize.y * 0.5f;
    const float exitCenterY = m_position.y - kExitOffsetY - kSlotSize.y * 0.5f;

    sf::RectangleShape busTrack;
    busTrack.setPosition({
        busCenterX - kTrackThickness * 0.5f,
        exitCenterY
    });
    ...
    for (std::size_t row = 0; row < rows; ++row)
    {
        ...
        const float laneEndX =
            m_position.x + kLeftPadding +
            static_cast<float>(columns - 1) * (kSlotSize.x + kColumnGap) +
            kSlotSize.x * 0.5f;

        sf::RectangleShape horizontalTrack;
        horizontalTrack.setPosition({
            busCenterX + kTrackTurnRadius,
            laneCenterY - kTrackThickness * 0.5f
        });
        ...
        m_trackBends.push_back(buildTrackBend(
            {busCenterX + kTrackTurnRadius, laneCenterY - kTrackTurnRadius},
            kTrackTurnRadius,
            kTrackThickness,
    ...
    for (std::size_t index = 0; index < m_lines.size(); ++index)
    {
        ...
        verticalTrack.setPosition({
            x + kSlotSize.x * 0.5f - kTrackThickness * 0.5f,
            y + kSlotSize.y
        });
        verticalTrack.setSize({
            kTrackThickness,
            laneCenterY - (y + kSlotSize.y)
        });
