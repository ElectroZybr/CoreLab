...
)
{
    ...
    m_toTurnEntryDuration = durationForDistance(length(m_turnEntryPosition - m_lanePosition), kInternalRouteSpeed);
    m_turnDuration =
...
void MemoryReadAnimation::update(float deltaSeconds)
{
    ...
    case Phase::Turn:
    {
        ...
        const sf::Vector2f center{
            m_turnCenter.x + std::cos(angle) * m_turnRadius,
            m_turnCenter.y + std::sin(angle) * m_turnRadius
        };
        m_copy.setPosition({
            center.x - CacheLine::kWidth * 0.5f,
            center.y - CacheLine::kHeight * 0.5f
        });
...
float MemoryReadAnimation::softEase(float t)
{
    ...
    return t - amplitude * std::sin(std::numbers::pi_v<float> * 2.0f * t) /
